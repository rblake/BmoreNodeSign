  ...
  if (Serial.available() >= 96) {
     for (int row=0; row<8; row++) {
       for (int col=0; col<8; col += 2) {
         byte buffer0 = Serial.read();
         byte buffer1 = Serial.read();
         byte buffer2 = Serial.read();
         //change the read bits into colors.
         SetPixel(row,col  ,buffer0 & 0xF0, buffer0 << 4,   buffer1 & 0xF0 );
         SetPixel(row,col+1,buffer1 << 4,   buffer2 & 0xF0, buffer2 << 4   );
       }
     }
  }
}
